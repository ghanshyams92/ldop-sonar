...
pipeline {
    ...
    stages {
        ...
        stage('ldop-sonar-validate') {
            ...
            steps {
                ...
                script {
                    ...
                    if (doesVersionExist('liatrio', 'ldop-sonar', TAG)) {
                        error("LDOP Sonar version already exists! Aborting...")
                    }
                }
            }
            post {
                ...
                always { script { SUBJECT = "Build #${env.BUILD_NUMBER} of ${env.JOB_NAME} at 'ldop-sonar-validate'" } }
            }
        }
        stage('hadolint-lint') {
            agent {
        ...
        stage('ldop-sonar-build') {
            agent any
            steps {
                script { CHANGED = "NO" }
                sh "printenv"
                sh "rm -rf test/integration/*"
                sh "docker build -t liatrio/ldop-sonar:${env.BRANCH_NAME} ."
                sh "docker push liatrio/ldop-sonar:${env.BRANCH_NAME}"
            }
            post {
                success { script { STATUS = "SUCCESS" } }
                failure { script { STATUS = "FAILURE" } }
                changed { script { CHANGED = "YES" } }
                always { script { SUBJECT = "Build #${env.BUILD_NUMBER} of ${env.JOB_NAME} at 'ldop-sonar-build'" } }
            }
        }
        stage('ldop-integration-testing') {
            ...
            steps {
                ...
                withCredentials([[
                    ...
                    testSuite("ldop-sonar", "${env.BRANCH_NAME}", DIR)
        ...
        stage('ldop-image-deploy') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-sonar:${env.BRANCH_NAME} liatrio/ldop-sonar:${TAG}"
                sh "docker push liatrio/ldop-sonar:${TAG}"
            }
            post {
                success { script { STATUS = "SUCCESS" } }
                failure { script { STATUS = "FAILURE" } }
