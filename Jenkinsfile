pipeline {
    ...
    stages {
            ...
            steps {
                ...
                script {
                    if ( env.BRANCH_NAME == 'master' ) {
                        containerVersion = getVersionFromContainer("liatrio/ldop-sonar:${env.BRANCH_NAME}")
                        failIfVersionExists("liatrio","ldop-sonar",containerVersion)
                        sh "docker build -t liatrio/ldop-sonar:${containerVersion} ."
                    }
                }
            }
            post {
        ...
        stage('Push to dockerhub') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-sonar:${env.BRANCH_NAME} liatrio/ldop-sonar:latest"
                sh "docker push liatrio/ldop-sonar:latest"
                script {
                    if ( env.BRANCH_NAME == 'master' )
                        sh "docker push liatrio/ldop-sonar:${containerVersion}"
                }
            }
            post {
                success { script { STATUS = "SUCCESS" } }
